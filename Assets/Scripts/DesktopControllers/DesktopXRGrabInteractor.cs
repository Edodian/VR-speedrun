...
public sealed class DesktopXRGrabInteractor : XRBaseInteractor
{
    ...
    private Ray BuildPickRay()
    {
        ...
        if (!useCenter && Mouse.current != null)
        {
            ...
            // Guard against NaN / Infinity. The Input System occasionally returns
            // non-finite values during cursor lock transitions or focus changes;
            // feeding those into ScreenPointToRay spams "screen pos -nan(ind)"
            // warnings every frame.
            if (IsScreenPositionValid(mousePos))
    ...
    {
        // float.IsFinite covers both NaN and +/-Infinity.
